...
//                
// Title:            mysh
// Files:            makefile, mysh.c
// Semester:         CS537 Spring 2014
//
// Author:           Sung Gon Kim , Gaopeng Wang 
// Email:            Kim376@wisc.edu, gwang63@wisc.edu
//
///////////////////////////////////////////////////////////////////////////
...
//used outside bin command
void runOutCommand(char * argv[], const char* loc)
{
	execvp(loc, argv);
	printError();
	exit(0);
...
//boolean check for if file extentino is py for phython file
...
//check for any unusal char in string
...
//if there is any unsual char count number
int countSpecial(char* args[1000], int size)
{
	char* target;
	int i;
	int count = 0;
...
void runCommand(char* args[10000], int size, int redirect, int background, int runPy)
{
	...
	else
	{
		...
		{
			if(pid == 0)
			{	//get python folder and run command
				if(runPy)
				{
					runOutCommand(argv, "/usr/bin/python");
				}
				else
				{
					runOutCommand(argv, args[0]);
				}
...
int main(int argc, char const *argv[])
{
	...
	while(1)
	{
		...
		{	
			...
			char tabDelims[] = "\t";
			char* space, *clean;
			char* result = strtok_r(cleanLine, delims, &space);
			char* args[10000];
			int size = 0;
			while(result != NULL)
			{
