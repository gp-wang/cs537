...
conn_nodes[i].fd=
  ...
  fd_holder=conn_nodes[0].fd= Open_clientfd(host, port);
  ...
  for(i=0;i<numClients;i++) {
    ...
    clientSend(conn_nodes[0].fd, filename);
    printf("\nRequest being sent out:  %d, fd: %d", conn_nodes[i].index, conn_nodes[0].fd);
    client_count++;
